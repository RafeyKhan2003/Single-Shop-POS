...
class PurchaseDB {
  constructor() {
    ...
    this.PurchaseDraft = Schema(`PurchaseDraft${suffix}`, {
  ...
  autoSavePurchaseDraft(data) {
    try {
      const existing = this.PurchaseDraft.findOne({ draft_id: 1 })
      const draftData = {
        draft_id: 1,
        cart: data.cart || [],
      ...
      if (existing) {
        ...
        this.PurchaseDraft.create(draftData).save()
      }
      return true
    } catch (e) {
      console.error('autoSavePurchaseDraft error:', e)
      return false
    }
  }

  getPurchaseDraft() {
    return this.PurchaseDraft.findOne({ draft_id: 1 }) || null
  }
